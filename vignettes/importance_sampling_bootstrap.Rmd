...
For the Bootstrap importance sampling, we need the log-likelihood for each observation.
...
log_lik_stanfit <- function(fit, upars, parameter_name = "log_lik",
    ...
    out <- matrix(0, S, n)
    for (s in seq_len(S)) {
      out[s, ] <- rstan::constrain_pars(fit, upars = upars[s, ])[[parameter_name]]
    }
    out
}
...
hist(bs_postmeans_is_mm$logsigma, col = rgb(1, 0, 0, 1 / 4), add = TRUE, freq = FALSE,
     breaks = seq(-0.1, 0.15, length.out = 30))
legend("topright", legend = c("model posterior", "Bootstrap posterior means"),
       fill = c(rgb(0, 0, 1, 1 / 4), rgb(1, 0, 0, 1 / 4)))
