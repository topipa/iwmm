...
We achieve this by defining a `log_ratio_fun`
which we can use to indicate the ratio of the target distribution and
the existing posterior distribution.
Since the target distribution is the posterior with
one observation removed, the ratio function is the inverse of the
likelihood of that observation. The log ratio function is thus
the negative log likelihood of the last observation.
