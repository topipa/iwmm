test_that("transformations work", {
  ...
  expect_equal(trans1$draws, matrix(c( 2.7129633, -0.1070367, -0.3070367, 1.3929633, 1.905564, 2.255564, -0.134436, -0.144436),4,2),tolerance = 1e-7)
  expect_equal(trans1$shift, c(0.962963269, -0.004435991),tolerance = 1e-7)

  trans2 <- shift_and_scale(draws, lw)
  expect_equal(trans2$draws, matrix(c( 2.24928499, 0.15977256, 0.01158019, 1.27121534, 1.84278570, 2.16928580, -0.06024344, -0.06957202),4,2),tolerance = 1e-7)
  ...
  expect_equal(trans3$draws, matrix(c(2.43067094, 0.05539964, -0.11305932,  1.31884182, 2.5189668, 1.6583828, -0.4939763, 0.1988827),4,2),tolerance = 1e-7)
  expect_equal(trans3$shift, c(0.962963269, -0.004435991),tolerance = 1e-7)
  expect_equal(trans3$mapping, matrix(c(0.8422948, 0.4126585, 0.0000000, 0.8660365),2,2),tolerance = 1e-7)
